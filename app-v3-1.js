(() => {
  ...
  function renderThemes() {
    ...
    const allCollapsed = themes.every(theme => collapsed.has(String(theme.id)));
    container.innerHTML = `<div class="theme-controls"><button class="text-button" data-theme-all="${allCollapsed ? "expand" : "collapse"}" type="button">${allCollapsed ? "Expand all" : "Collapse all"}</button></div>` + themes.map(theme => {
  ...
  function renderAppHealth() {
    ...
    if (workerURL && !workerURL.includes("service-worker-v3-1.js")) issues.push({ severity: "warning", code: "worker_version", message: "An older offline worker is still controlling this screen. Close and reopen Focus once.", repairable: false });
  ...
  function exportData() {
    ...
    anchor.href = url;
    anchor.download = `focus-backup-v3-1-${localISO()}.json`;
  ...
  document.addEventListener("click", event => {
    ...
    const allThemes = event.target.closest("[data-theme-all]"); if (allThemes) {
      state.settings.collapsedThemes = allThemes.dataset.themeAll === "collapse" ? state.themes.filter(theme => themeTargets(theme.id).length).map(theme => String(theme.id)) : [];
      saveState();
    }
  ...
  if ("serviceWorker" in navigator) window.addEventListener("load", () => navigator.serviceWorker.register("./service-worker-v3-1.js", { updateViaCache: "none" }).catch(console.error));
  ...
  ensureDailySnapshot().then(async () => {
    if (migrationSnapshotPending) {
      await createRecoverySnapshot("After successful Focus 3.1 migration");
