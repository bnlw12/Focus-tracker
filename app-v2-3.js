(() => {
  ...
  function migrateState(candidate) {
    ...
    const isV2 = Number(candidate.version) >= 2;
    if (!isV2 && !localStorage.getItem(PRE_V2_BACKUP_KEY)) localStorage.setItem(PRE_V2_BACKUP_KEY, JSON.stringify(candidate));

    const currentFocus = normaliseFocus(candidate.currentFocus, "My 90-day focus");
    const focusHistory = (Array.isArray(candidate.focusHistory) ? candidate.focusHistory : []).map((focus, index) => normaliseFocus(focus, `Focus period ${index + 1}`));
    const themes = isV2 && Array.isArray(candidate.themes) && candidate.themes.length ? deepClone(candidate.themes) : deepClone(DEFAULT_THEMES);
    DEFAULT_THEMES.forEach(theme => { if (!themes.some(item => sameId(item.id, theme.id))) themes.push(deepClone(theme)); });
    const normalisedThemes = themes.map(theme => ({ ...theme, id: String(theme.id || uid("theme")) }));
    ...
    const events = candidate.events.map(event => {
      const date = normaliseDate(event.date, normaliseDate(event.createdAt, localISO()));
      return {
        ...deepClone(event),
    ...
    return {
      ...
      targets,
      events,
      contextEntries: (isV2 && Array.isArray(candidate.contextEntries) ? candidate.contextEntries : []).map(entry => ({
        ...deepClone(entry), id: String(entry.id || uid("context")), date: normaliseDate(entry.date, localISO())
      })),
      settings: { briefingThreshold: 10, contextEnabled: true, analysisTargetId: "", ...(isV2 ? candidate.settings || {} : {}) },
      meta: { migratedAt: isV2 ? null : new Date().toISOString(), lastExportAt: null, persistentStorage: null, ...(isV2 ? candidate.meta || {} : {}) }
    };
  }
  ...
  function renderManage() {
    ...
    if (versionText) versionText.textContent = `Focus ${APP_VERSION} · Local-first · No account or cloud database.`;
    const dataStatus = $("#dataStatus");
    if (dataStatus) dataStatus.textContent = `${state.events.length} stored entries · ${currentStored} in the current focus${lastEvent ? ` · Last log ${formatDate(eventDate(lastEvent))}` : ""}.`;
  ...
  function addEntries(targetId, date, quantity = 1) {
    ...
    for (let index = 0; index < safeQuantity; index += 1) {
      state.events.push({
        ...
        focusId: String(state.currentFocus.id),
        date: safeDate,
        createdAt: new Date().toISOString()
      });
    }
    const focusCount = countForFocusTarget(target.id, state.currentFocus);
    const saved = saveState(`${target.name}: ${focusCount} completed`);
    if (!saved) state.events.splice(beforeLength);
  }
  ...
  $("#targetForm").addEventListener("submit", event => {
    ...
    if (id) Object.assign(state.targets.find(target => sameId(target.id, id)), details);
  ...
  if ("serviceWorker" in navigator) window.addEventListener("load", () => navigator.serviceWorker.register("./service-worker-v2-3.js").catch(console.error));
